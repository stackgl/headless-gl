...
const HEADLESS_VERSION = require('../package.json').version as string;
import { gl, NativeWebGLRenderingContext, NativeWebGL } from './native-gl';
import { getANGLEInstancedArrays } from './extensions/angle-instanced-arrays';
import { getOESElementIndexUint } from './extensions/oes-element-index-unit';
import { getOESStandardDerivatives } from './extensions/oes-standard-derivatives';
import { getOESTextureFloat } from './extensions/oes-texture-float';
import { getOESTextureFloatLinear } from './extensions/oes-texture-float-linear';
import { getSTACKGLDestroyContext } from './extensions/stackgl-destroy-context';
import { getSTACKGLResizeDrawingBuffer } from './extensions/stackgl-resize-drawing-buffer';
import { getWebGLDrawBuffers } from './extensions/webgl-draw-buffers';
import { getEXTBlendMinMax } from './extensions/ext-blend-minmax';
import { getEXTTextureFilterAnisotropic } from './extensions/ext-texture-filter-anisotropic';
import { getEXTShaderTextureLod } from './extensions/ext-shader-texture-lod';
import { getOESVertexArrayObject } from './extensions/oes-vertex-array-object';
import {
  bindPublics,
  checkObject,
  checkUniform,
...
import { WebGLActiveInfo } from './webgl-active-info';
import { WebGLFramebuffer } from './webgl-framebuffer';
import { WebGLBuffer } from './webgl-buffer';
import { WebGLDrawingBufferWrapper } from './webgl-drawing-buffer-wrapper';
import { WebGLProgram } from './webgl-program';
import { WebGLRenderbuffer } from './webgl-renderbuffer';
import { WebGLShader } from './webgl-shader';
import { WebGLShaderPrecisionFormat } from './webgl-shader-precision-format';
import { WebGLTexture } from './webgl-texture';
import { WebGLUniformLocation } from './webgl-uniform-location';
import { WebGLVertexArrayObject } from './webgl-vertex-array-object';
import { getEXTColorBufferFloat } from './extensions/ext-color-buffer-float';
...
const availableExtensions: Record<string, (ctx: any) => any> = {
...
export function wrapContext(ctx: any): any {
...
class WebGLRenderingContextHelper extends NativeWebGLRenderingContext {
  _checkLocation(location: any): boolean {
  ...
  _checkLocationActive(location: any): boolean {
    if (!location) {
      return false;
  ...
  _checkShaderSource(_shader: any): boolean {
    return true;
  }

  _checkTextureTarget(target: number): boolean {
  ...
  _fixupLink(program: any): boolean {
  ...
  _getActiveBuffer(target: number): any {
  ...
  _getActiveTextureUnit(): any {
    return this._textureUnits[this._activeTextureUnit];
  }

  _getActiveTexture(target: number): any {
  ...
  _getActiveFramebuffer(target: number): any {
  ...
  _getParameterDirect(pname: number): any {
    return super.getParameter(pname);
  }

  _getTexImage(target: number): any {
    const unit = this._getActiveTextureUnit();
    if (target === this.TEXTURE_2D) {
      return unit._bind2D;
  ...
  _isConstantColorBlendFunc(factor: number): boolean {
    return (
      factor === this.CONSTANT_COLOR || factor === this.ONE_MINUS_CONSTANT_COLOR
    );
  }

  _isConstantAlphaBlendFunc(factor: number): boolean {
    return (
      factor === this.CONSTANT_ALPHA || factor === this.ONE_MINUS_CONSTANT_ALPHA
    );
  }

  _isObject(object: any, method: string, Wrapper: any): boolean {
    if (
  ...
  _restoreError(lastError: number): void {
  ...
  _switchActiveProgram(active: any): void {
    if (active) {
      active._refCount -= 1;
      active._checkDelete();
    }
  ...
  _validBlendFunc(factor: number): boolean {
    return (
      factor === this.ZERO ||
      factor === this.ONE ||
  ...
  _validBlendMode(mode: number): boolean {
    return (
      mode === this.FUNC_ADD ||
  ...
  _validCubeTarget(target: number): boolean {
    return (
  ...
  _validFramebufferAttachment(attachment: number): boolean {
    switch (attachment) {
  ...
  _validTextureTarget(target: number): boolean {
    return target === this.TEXTURE_2D || target === this.TEXTURE_CUBE_MAP;
  }

  _verifyTextureCompleteness(
    target: number,
    pname: number,
    param: number,
  ): void {
  ...
  _wrapShader(_type: number, source: string): string {
    return source;
  }
  ...
  bindRenderbuffer(target: number, object: any): void {
  ...
  bindVertexArray(array: any): void {
  ...
  checkFramebufferStatus(target: number): any {
  ...
  colorMask(red: any, green: any, blue: any, alpha: any): any {
  ...
  deleteVertexArray(array: any): void {
    if (!checkObject(array)) {
  ...
  getActiveAttrib(program: any, index: number): WebGLActiveInfo | null {
  ...
  getActiveUniform(program: any, index: number): WebGLActiveInfo | null {
    if (!checkObject(program)) {
  ...
  getAttachedShaders(program: any): any[] | null {
    if (
      !checkObject(program) ||
  ...
  ): WebGLShaderPrecisionFormat | null | undefined {
    shaderType |= 0;
    precisionType |= 0;
  ...
  getProgramInfoLog(program: any): string | null {
    if (!checkObject(program)) {
  ...
  getShaderInfoLog(shader: any): string | null {
    if (!checkObject(shader)) {
  ...
  getShaderSource(shader: any): string | null {
    if (!checkObject(shader)) {
  ...
  getUniform(program: any, location: any): any {
  ...
  getUniformLocation(
    program: any,
    name: string,
  ): WebGLUniformLocation | null | undefined {
    if (!checkObject(program)) {
  ...
  getVertexAttribOffset(index: number, pname: number): any {
    index |= 0;
    pname |= 0;
    if (index < 0 || index >= this._vertexObjectState._attribs.length) {
      this.setError(this.INVALID_VALUE);
  ...
  sampleCoverage(value: number, invert: any): any {
  ...
  uniform1f(location: any, v0: number): void {
  ...
  uniform1i(location: any, v0: number): void {
  ...
  uniform2f(location: any, v0: number, v1: number): void {
  ...
  uniform2i(location: any, v0: number, v1: number): void {
  ...
  uniform3f(location: any, v0: number, v1: number, v2: number): void {
  ...
  uniform3i(location: any, v0: number, v1: number, v2: number): void {
  ...
  vertexAttrib1f(index: number, v0: number): any {
    index |= 0;
  ...
  vertexAttrib2f(index: number, v0: number, v1: number): any {
    index |= 0;
    if (!this._checkVertexIndex(index)) return;
    const data = this._vertexGlobalState._attribs[index]._data;
    data[3] = 1;
  ...
  vertexAttrib3f(index: number, v0: number, v1: number, v2: number): any {
    index |= 0;
    if (!this._checkVertexIndex(index)) return;
    const data = this._vertexGlobalState._attribs[index]._data;
    data[3] = 1;
  ...
  vertexAttrib1fv(index: number, value: any): any {
  ...
  vertexAttrib2fv(index: number, value: any): any {
  ...
  vertexAttrib3fv(index: number, value: any): any {
  ...
  vertexAttrib4fv(index: number, value: any): any {
...
export class WebGLRenderingContext extends WebGLRenderingContextHelper {}

export class WebGL2RenderingContext extends WebGLRenderingContextHelper {}
